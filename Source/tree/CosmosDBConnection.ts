	...
	public async deleteTreeItemImpl(context: IActionContext): Promise<void> {
		...
		if (appSettings.properties) {
			const warning: string = localize(
				"removeConnection",
				'Are you sure you want to remove connection "{0}"? This will delete the following application settings: {1}',
				this.label,
				this.appSettingKeys.map((s) => `"${s}"`).join(", "),
			);
