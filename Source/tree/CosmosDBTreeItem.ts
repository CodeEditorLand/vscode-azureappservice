...
export class CosmosDBTreeItem extends AzExtParentTreeItem {
	public static contextValueInstalled = "сosmosDBConnections";
	public static contextValueNotInstalled = "сosmosDBNotInstalled";
	...
	public get contextValue(): string {
		return this.cosmosDBExtension
			? CosmosDBTreeItem.contextValueInstalled
			: CosmosDBTreeItem.contextValueNotInstalled;
	}
	...
	public async loadMoreChildrenImpl(
		...
		if (!this.cosmosDBExtension) {
			return [
				new GenericTreeItem(this, {
					...
					label: localize(
						"installDb",
						"Install Azure Databases Extension...",
					),
				}),
			];
		}
		...
		const connections: IDetectedConnection[] = this.detectMongoConnections(
			...
			.concat(this.detectDocDBConnections(appSettings))
			.concat(this.detectPostgresConnections(appSettings));
		...
		if (treeItems.length > 0) {
			...
			return [
				new GenericTreeItem(this, {
					...
					label: localize(
						"addDbConnection",
						"Add Azure Databases Connection...",
					),
				}),
			];
		}
	...
	public async createChildImpl(
		...
		if (databaseToAdd.docDBData) {
			const docdbAppSettings = new Map([
				[
					...
					nonNullProp(databaseToAdd, "docDBData").documentEndpoint,
				],
				[
					this._keySuffix,
					nonNullProp(databaseToAdd, "docDBData").masterKey,
				],
		...
		const message: string = localize(
			...
			createdDatabase.label,
			this.parent.site.fullName,
			Array.from(newAppSettings.keys()).join(", "),
		);
		// Don't wait
	...
	private detectMongoConnections(appSettings: {
	...
	private detectPostgresConnections(appSettings: {
		[propertyName: string]: string;
	}): IDetectedConnection[] {
	...
	private detectDocDBConnections(appSettings: {
		[propertyName: string]: string;
	}): IDetectedConnection[] {
	...
	private async promptForAppSettings(
		context: IActionContext,
		appSettingsDict: StringDictionary,
		accountAppSettings: Map<string | undefined, string | undefined>,
		suffixes: string[] | undefined,
		defaultPrefixString: string,
		...
		const appSettingsPrefix: string = await context.ui.showInputBox({
			...
			validateInput: (v: string): string | undefined => {
				if (v) {
					return this.validateAppSettingPrefix(
						client,
						v,
						appSettingsDict,
						suffixes,
	...
	private getAppSettings(
		appSettings: Map<string | undefined, string | undefined>,
	...
	private validateAppSettingPrefix(
		...
		if (suffixes) {
			return suffixes.reduce<string | undefined>(
				(result, suffix) =>
					result ||
					validateAppSettingKey(
						appSettingsDict,
