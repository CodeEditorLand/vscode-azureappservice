...
export async function downloadAppSettings(
	...
	void window
		.showInformationMessage(
			localize(
				"Settings",
				'Downloaded settings from "{0}".  View settings file?',
				client.fullName,
			),
			localize("view", "View file"),
		)
		.then(async (input) => {
			if (!input) {
...
export function convertAppSettingsToEnvVariables(
	appSettings: { [propertyName: string]: string },
	appName: string,
): string {
	let envData: string = localize(
		"envComment",
		'# Downloaded Application Settings from "{0}"{1}',
		appName,
		os.EOL,
	);
