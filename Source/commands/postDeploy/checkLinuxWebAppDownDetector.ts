...
export async function checkLinuxWebAppDownDetector(
	...
	return await callWithTelemetryAndErrorHandling(
		...
		async (context: IActionContext): Promise<void> => {
			...
			const deployment: DeployResult = await kuduClient.getDeployResult(
				context,
				"latest"
			);
			...
			while (!detectorErrorMessage) {
				...
				const startTime: string = dayjs
					.utc(startTimeDate)
					.format(detectorTimestampFormat);
				const endTime: string = dayjs
					.utc(endTimeDate)
					.format(detectorTimestampFormat);
				const deployEndTime: string = dayjs
					.utc(deployment.endTime)
					.format(detectorTimestampFormat);

				detectorErrorMessage = await getLinuxDetectorError(
					context,
					linuxLogViewer,
					node,
					startTime,
			...
			void context.ui
				.showWarningMessage(detectorErrorMessage, {
					title: localize("viewDetails", "View details"),
				})
				.then(async () => {
					await callWithTelemetryAndErrorHandling(
						"viewedDetectorDetails",
						async (context2: IActionContext) => {
							...
							await openInPortal(
								...
								{
									queryPrefix: `websitesextension_ext=asd.featurePath%3Ddetectors%2F${linuxLogViewer}`,
								}
							);
						}
					);
