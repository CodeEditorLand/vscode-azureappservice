...
export async function getLinuxDetectorError(
	...
	const response: AzExtPipelineResponse = <AzExtPipelineResponse>(
		await client.sendRequest(
			createPipelineRequest({
				method: "GET",
				url: createRequestUrl(detectorUri, {
	...
	const responseJson: detectorResponseJSON = <detectorResponseJSON>(
		response.parsedBody
	);
	...
	const insightLogTable: detectorTable | undefined = findTableByName(
		responseJson.properties.dataset,
		"insight/logs",
	);
	...
	const rawApplicationLog: string = getValuesByColumnName(
		context,
		insightLogTable,
		ColumnName.value,
	);
	...
	if (appInsightTable) {
		...
		const dockerInsightTable: detectorTable | undefined = findTableByName(
			insightDataset,
			"docker/insight",
		);
	...
	if (
		!detectorTimestamp ||
		!validateTimestamp(context, detectorTimestamp, deployEndTime)
	) {
		return undefined;
	}

	if (
		getValuesByColumnName(context, insightTable, ColumnName.status) ===
		"Critical"
	) {
		const insightError: string = getValuesByColumnName(
			context,
			insightTable,
...
export function validateTimestamp(
	context: IActionContext,
	detectorTime: string,
	deployResultTime: string,
): boolean {
	const secondsBetweenTimes: number =
		(new Date(detectorTime).getTime() -
			new Date(deployResultTime).getTime()) /
		1000;
	// the detector can be as fast as ~20 seconds for app errors, but Docker container errors seem to timeout at
	// about 5 minutes
	context.telemetry.properties.timeBetweenDeployAndDetector =
		secondsBetweenTimes.toString();
