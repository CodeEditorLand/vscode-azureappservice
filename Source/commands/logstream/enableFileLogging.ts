...
export async function enableFileLogging(
	context: IEnableFileLoggingContext,
	node?: SiteTreeItem | LogFilesTreeItem,
): Promise<void> {
	if (!node) {
	...
	const isEnabled: boolean = await vscode.window.withProgress(
	...
	if (!isEnabled && siteNode instanceof SiteTreeItem) {
		await context.ui.showWarningMessage(
			localize(
				"enableLogging",
				'Do you want to enable file logging for "{0}"? The web app will be restarted.',
		...
		void vscode.window.showInformationMessage(
			localize(
				"loggingEnabled",
				'File logging has already been enabled for "{0}".',
				siteNode.site.fullName,
			),
		);
	}
