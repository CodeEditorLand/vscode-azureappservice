...
export async function generateDeploymentScript(
	...
	await window.withProgress(
		...
		async (p) => {
			p.report({
				message: localize("generatingScript", "Generating script..."),
			});
			...
			if (!siteConfig.linuxFxVersion) {
				...
				const containerCmdParameters =
					(serverUrl
						...
						: "") +
					(serverUser
						? "--docker-registry-server-user $SERVERUSER "
						: "") +
					(serverPwd
				...
				script = scriptTemplate
					...
					.replace(
						"%IMAGENAME%",
						siteConfig.linuxFxVersion.substring(
							siteConfig.linuxFxVersion.indexOf("|") + 1,
						),
					)
			...
			script = script
				.replace(
					"%SUBSCRIPTION_NAME%",
					node.subscription.subscriptionDisplayName,
				)
