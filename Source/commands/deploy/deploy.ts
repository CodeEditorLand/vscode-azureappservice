...
const postDeployCancelTokens: Map<string, vscode.CancellationTokenSource> =
	new Map<string, vscode.CancellationTokenSource>();

export async function deploy(
	actionContext: IActionContext,
	arg1?: vscode.Uri | SiteTreeItem,
	arg2?: (vscode.Uri | SiteTreeItem)[],
	isNewApp = false,
): Promise<void> {
	...
	const fileExtensions: string | string[] | undefined =
		await javaUtils.getJavaFileExtensions(siteConfig);
	...
	const isZipDeploy: boolean =
		siteConfig.scmType !== constants.ScmType.LocalGit &&
		siteConfig.scmType !== constants.ScmType.GitHub;
	...
	if (
		getWorkspaceSetting<boolean>(
			constants.configurationSettings.showBuildDuringDeployPrompt,
		...
		if (
			!hasSCMDoBuildSetting &&
			siteConfig.linuxFxVersion &&
			isZipDeploy &&
			!(await pathExists(
				path.join(
					context.effectiveDeployFsPath,
					constants.deploymentFileName,
				),
			))
		) {
	...
	void promptToSaveDeployDefaults(
		context,
		node,
		context.workspaceFolder.uri.fsPath,
		context.effectiveDeployFsPath,
	...
	const deployPath: string = isZipDeploy
		? context.effectiveDeployFsPath
		: context.originalDeployFsPath;

	if (
		!isZipDeploy &&
		isPathEqual(context.effectiveDeployFsPath, context.originalDeployFsPath)
	) {
		const noSubpathWarning: string = localize(
			"ignoreSuppath",
	...
	await node.runWithTemporaryDescription(
		...
		async () => {
			try {
				await appservice.deploy(
					nonNullValue(node).site,
					<string>deployPath,
					{
						...context,
