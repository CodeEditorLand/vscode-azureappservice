...
export async function promptToSaveDeployDefaults(
	context: IActionContext,
	node: SiteTreeItem,
	workspacePath: string,
	deployPath: string,
): Promise<void> {
	const defaultWebAppToDeploySetting: string | undefined =
		getWorkspaceSetting(
	...
	if (defaultWebAppToDeploySetting) {
		context.telemetry.properties.promptToSaveDeployConfigs =
			defaultWebAppToDeploySetting === constants.none
				? constants.none
				: "usesDefault";
	} else {
		const saveDeploymentConfig: string = localize(
			"showDeploymentConfig",
			'Always deploy the workspace "{0}" to "{1}"?',
			path.relative(workspacePath, deployPath)
				? deployPath
...
export async function saveDeployDefaults(
	nodeFullId: string,
	workspacePath: string,
	deployPath: string,
): Promise<void> {
