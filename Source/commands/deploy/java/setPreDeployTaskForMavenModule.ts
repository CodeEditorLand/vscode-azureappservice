...
export async function setPreDeployTaskForMavenModule(
	...
	const relativeModulePath: string = path.relative(
		workspaceFspath,
		module.path,
	);
	...
	const deploySubpath: string = path.posix.join(
		relativeModulePath,
		"target",
		module.artifactFinalName,
	);

	await updateWorkspaceSetting(
		constants.configurationSettings.preDeployTask,
	...
	context.effectiveDeployFsPath = path.posix.join(
		workspaceFspath,
		deploySubpath,
	);
...
function generateMavenPackageTask(relativeModulePath: string): TaskDefinition {
	...
	const cwd: string = path.posix.join(
		"${workspaceFolder}",
		relativeModulePath,
	);
