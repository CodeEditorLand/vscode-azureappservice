...
import { type MessageItem, Uri, env, window } from "vscode";
...
async function showSurveyPopup(message: string, uri: Uri): Promise<void> {
	const button: MessageItem | undefined = await window.showErrorMessage(
		message,
...
export function failureMoreInfoSurvey(
	error: IParsedError,
	siteConfig: SiteConfigResource,
): boolean {
	...
	void showSurveyPopup(
		...
		Uri.parse(
			`${SURVEY_URL}?k=${encodeURIComponent(
				failureKey,
