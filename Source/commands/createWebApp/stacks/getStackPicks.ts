...
export async function getStackPicks(
	context: IWebAppWizardContext,
	javaVersion?: string,
): Promise<IAzureQuickPickItem<FullWebAppStack | FullJavaStack>[]> {
	const stacks: WebAppStack[] = (await getStacks(context)).filter(
		(s) =>
			(javaVersion && s.value === "javacontainers") ||
			(!javaVersion && s.value !== "javacontainers"),
	);
	...
	for (const stack of stacks) {
		for (const majorVersion of stack.majorVersions) {
			...
			const minorVersions: AppStackMinorVersion<
				WebAppRuntimes & JavaContainers
			>[] = majorVersion.minorVersions.filter(
				(mv) => !/\..*\./.test(mv.value),
			);
	...
	if (context.usingBackupStacks) {
		...
		picks.unshift({
			label: localize(
				"backupStacksWarning",
				"$(warning) Failed to retrieve latest stacks. This list may be out of date.",
			),
			onPicked: () => {
				/* do nothing */
			},
...
function getFlagOs(
	ss: WebAppRuntimes & JavaContainers,
	key: "isPreview" | "isEarlyAccess",
): FlagOS {
	if (
		[
			ss.linuxContainerSettings,
			ss.windowsContainerSettings,
			ss.linuxRuntimeSettings,
			ss.windowsRuntimeSettings,
		].every((s) => !s || s[key])
	) {
		...
		ss.linuxRuntimeSettings?.[key] ||
		ss.linuxContainerSettings?.[key]
	) {
		return "Linux";
	} else if (
		ss.windowsRuntimeSettings?.[key] ||
		ss.windowsContainerSettings?.[key]
	) {
		return "Windows";
	} else {
		return "None";
...
async function getStacks(
	context: IWebAppWizardContext & { _stacks?: WebAppStack[] },
): Promise<WebAppStack[]> {
	if (!context._stacks) {
