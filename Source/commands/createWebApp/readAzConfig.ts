 ...
 */
export async function readAzConfig(
	wizardContext: IWebAppWizardContext & Partial<IDeployContext>,
	...
	try {
		const azGlobalConfigListing: IAzConfigItem[] | undefined = <
		...
		>JSON.parse(
			await cpUtils.executeCommand(
				undefined,
				workspacePath,
				"az configure --list-defaults --output json --scope local",
			),
		);
...
function readAzConfigListing(
	wizardContext: IWebAppWizardContext,
	propertiesToRead: AzConfigProperty[],
	azConfigListing: IAzConfigItem[] | undefined,
	config: AzConfig,
): void {
	if (azConfigListing) {
