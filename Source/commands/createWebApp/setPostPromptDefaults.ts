...
import { type AzConfig, AzConfigProperty, readAzConfig } from "./readAzConfig";
...
export async function setPostPromptDefaults(
	...
	siteNameStep: SiteNameStep,
): Promise<void> {
	// Reading az config should always happen after prompting because it can cause a few seconds delay
	const config: AzConfig = await readAzConfig(
		wizardContext,
		AzConfigProperty.group,
		AzConfigProperty.location,
	);
	...
	let defaultName = `appsvc_${wizardContext.newSiteOS}_${location.name}`;
	const newSkuTier = nonNullProp(
		nonNullProp(wizardContext, "newPlanSku"),
		"tier",
	).toLowerCase();
	...
	try {
		const client: WebSiteManagementClient =
			await createWebSiteClient(wizardContext);
		const asp: AppServicePlan | undefined = await tryGetAppServicePlan(
			client,
			defaultGroupName,
			defaultPlanName,
		);
		...
		if (asp && (hasPerfDrop || !matchesTier(asp, newSkuTier))) {
			...
			if (
				...
				const allAppServicePlans: AppServicePlan[] =
					await uiUtils.listAllIterator(
						client.appServicePlans.list(),
					);
				const defaultPlans: AppServicePlan[] =
					allAppServicePlans.filter((plan) => {
						return (
							plan.name &&
							plan.name.includes(defaultPlanName) &&
							getResourceGroupFromId(
								nonNullProp(plan, "id"),
							).includes(defaultGroupName)
						);
					});
				...
				for (const plan of defaultPlans) {
					if (plan.name) {
						const groupName: string = getResourceGroupFromId(
							nonNullProp(plan, "id"),
						);
						const fullPlanData: AppServicePlan | undefined =
						...
						if (
							fullPlanData &&
							matchesTier(fullPlanData, newSkuTier) &&
				...
				wizardContext.newResourceGroupName =
					wizardContext.newResourceGroupName ||
					(await siteNameStep.getRelatedName(
						wizardContext,
						defaultGroupName,
					));
				...
				wizardContext.newPlanName = await siteNameStep.getRelatedName(
					wizardContext,
					defaultPlanName,
				);
				if (!wizardContext.newPlanName) {
					throw new Error(
						localize(
							"noUniqnueNameAsp",
							"Failed to generate unique name for app service plan. Use advanced creation to manually enter plan names.",
						),
					);
				}
			}
...
function checkPlanForPerformanceDrop(asp: AppServicePlan | undefined): boolean {
	...
	if (
		asp &&
		asp.numberOfSites !== undefined &&
		asp.numberOfSites >= maxNumberOfSites
	) {
...
async function promptPerformanceWarning(
	context: IActionContext,
	asp: AppServicePlan,
): Promise<void> {
	context.telemetry.properties.performanceWarning = "true";
	const showPlanPerformanceWarningSetting: string =
		"showPlanPerformanceWarning";
	const showPerfWarning: boolean | undefined = getWorkspaceSetting(
		showPlanPerformanceWarningSetting,
	);
	...
	if (showPerfWarning) {
		...
		const createAnyway: MessageItem = {
			title: localize("createAnyway,", "Create anyway"),
		};
		const message: string = localize(
			"tooManyPlansWarning",
			'The selected plan currently has {0} apps. Deploying more than {1} apps may degrade the performance on the apps in the plan.  Use "Create Web App... (Advanced)" to change the default resource names.',
			numberOfSites,
			maxNumberOfSites,
		);
		const input: MessageItem = await context.ui.showWarningMessage(
			message,
			{ modal: true, stepName: "showPerfWarning" },
			createAnyway,
