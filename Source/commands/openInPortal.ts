...
export async function openInPortal(
	...
	if (
		...
		await uiOpenInPortal(
			node,
			`${nonNullProp(node, "parent").id}/deploymentSlotsV2`
		);
		return;
	}
	...
	if (
		matchContextValue(node.contextValue, [
			new RegExp(DeploymentsTreeItem.contextValueConnected),
			new RegExp(DeploymentsTreeItem.contextValueUnconnected),
		])
	) {
	...
	if (
		matchContextValue(node.contextValue, [
			new RegExp(DeploymentTreeItem.contextValue),
		])
	) {
		await uiOpenInPortal(
			node,
			`${nonNullProp(node, "parent").parent
				?.id}/Deployments/${nonNullProp(node, "id")}`
		);
		return;
	}
