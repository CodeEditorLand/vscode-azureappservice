	...
	if (currentSshTerminal) {
		if (currentSshTerminal.starting) {
			throw new Error(
				localize(
					"sshStartedError",
					'Azure SSH is currently starting or already started for "{0}".',
					node.site.fullName
				)
			);
		} else if (
			currentSshTerminal.tunnel &&
			currentSshTerminal.localPort !== undefined
		) {
			connectToTunnelProxy(
				node,
				currentSshTerminal.tunnel,
...
async function startSshInternal(
	context: IActionContext,
	node: SiteTreeItem
): Promise<void> {
	if (!node.site.isLinux) {
		throw new Error(
			localize(
				"sshLinuxError",
				"Azure SSH is only supported for Linux web apps."
			)
		);
	}
	...
	await vscode.window.withProgress(
		...
		async (progress, token): Promise<void> => {
			reportMessage(
				localize("checking", "Checking app settings..."),
				progress,
				token
			);
			...
			reportMessage(
				localize("initSsh", "Initializing SSH..."),
				progress,
				token
			);
			...
			reportMessage(
				localize("connectingSsh", "Connecting to SSH..."),
				progress,
				token
			);
...
function connectToTunnelProxy(
	...
	const verifyPassPrompt = vscode.window.onDidWriteTerminalData(
	...
	const onCloseEvent: vscode.Disposable = vscode.window.onDidCloseTerminal(
		(e: vscode.Terminal) => {
			if (e.processId === terminal.processId) {
				...
				ext.outputChannel.appendLog(
					localize(
						"sshDisconnected",
						'Azure SSH for "{0}" has disconnected.',
						node.site.fullName
					)
				);
