...
suite("Detector Dataset Parser", () => {
	test("Find table by table name", () => {
		const table: detectorTable | undefined = findTableByName(
			detectorResponse.properties.dataset,
			"insight/logs"
		);
	...
	test("Get values by column name", async () => {
		...
		assert.strictEqual(
			rawApplicationLog,
			detectorResponse.properties.dataset[1].table.rows[0][3]
		);
	});
	...
	test("Verify validateTimestamp", async () => {
		...
		const appInsightTable: any = JSON.parse(
			detectorResponse.properties.dataset[1].table.rows[0][3]
		)[0].table;
		const detectorTimestamp: string = getValuesByColumnName(
			context,
			appInsightTable,
			ColumnName.dataName
		).replace(bracketsAndSpace, "");

		assert.strictEqual(
			validateTimestamp(context, detectorTimestamp, expectedTimestamp),
			true
		);
		assert.strictEqual(
			validateTimestamp(context, detectorTimestamp, staleTimestamp),
			true
		);
		assert.strictEqual(
			validateTimestamp(context, detectorTimestamp, futureTimestamp),
			false
		);
	});
...
const detectorResponse: any = {
	...
	properties: {
		metadata: {
			...
			author: "",
			description:
				"Check the summary of your latest logs. This allows you to view the relevant logs from your app, and highlights the specific text in the logs that might be a potential issue.",
			type: "Detector",
			supportTopicList: [],
			analysisTypes: ["LinuxAppDown"],
			score: 0.0,
