...
suite("Create Web App and deploy", function (this: Mocha.Suite): void {
	...
	const testCases: ITestCase[] = [
		...
		},
		{
			...
			versions: [
				...
				}, // Not sure why this fails on mac build machines - worth investigating in the future
				{
					version: "6.0",
					supportedAppOs: "Both",
					displayText: "6 (LTS)",
	...
	for (const testCase of testCases) {
		for (const version of testCase.versions) {
			const runtime: string = `${testCase.runtimePrefix} ${
				version.displayText || version.version
			}`;
			const promptForOs: boolean = version.supportedAppOs === "Both";
			const oss: string[] = promptForOs
				? ["Windows", "Linux"]
				: [version.supportedAppOs];
			for (const os of oss) {
				if (
					version.appOsToSkip !== os &&
					version.buildMachineOsToSkip !== process.platform
				) {
					parallelTests.push({
						title: `${runtime} - ${os}`,
						callback: async () => {
							const testFolderPath: string = getWorkspacePath(
								testCase.workspaceFolder || version.version
							);
							await testCreateWebAppAndDeploy(
								os,
								promptForOs,
								runtime,
								testFolderPath,
	...
	async function testCreateWebAppAndDeploy(
		os: string,
		promptForOs: boolean,
		runtime: string,
		...
		await runWithTestActionContext("Deploy", async (context) => {
			await context.ui.runWithInputs(
				[workspacePath, resourceName, "Deploy"],
				async () => {
		...
		const hostUrl: string | undefined = (<SiteTreeItem>(
		...
		const response: AzExtPipelineResponse = await client.sendRequest(
			createPipelineRequest({ method: "GET", url: hostUrl })
		);
