...
suite("Web App actions", function (this: Mocha.Suite): void {
	...
	const WebsiteOS0: WebsiteOS =
		new Date().getDate() % 2 === 0 ? WebsiteOS.linux : WebsiteOS.windows;
	const WebsiteOS1: WebsiteOS =
		WebsiteOS0 === WebsiteOS.windows ? WebsiteOS.linux : WebsiteOS.windows;
	...
	test(`Configure Deployment Source to LocalGit for ${WebsiteOS0} Web App`, async () => {
		let createdApp: SiteConfigResource =
			await webSiteClient.webApps.getConfiguration(
				resourceName,
				resourceName
			);
		assert.notStrictEqual(
			createdApp?.scmType,
			constants.ScmType.LocalGit,
			`Web App scmType's property value shouldn't be ${createdApp?.scmType} before "Configure Deployment Source to LocalGit".`
		);
		await runWithTestActionContext(
			"ConfigureDeploymentSource",
			async (context) => {
				await context.ui.runWithInputs(
					[resourceName, constants.ScmType.LocalGit],
					async () => {
		...
		createdApp = await webSiteClient.webApps.getConfiguration(
			resourceName,
			resourceName
		);
		assert.strictEqual(
			createdApp?.scmType,
			constants.ScmType.LocalGit,
			`Web App scmType's property value should be ${constants.ScmType.LocalGit} rather than ${createdApp?.scmType}.`
		);
	});
	...
	test(`Configure Deployment Source to None for ${WebsiteOS0} Web App`, async () => {
		...
		assert.notStrictEqual(
			createdApp?.scmType,
			constants.ScmType.None,
			`Web App scmType's property value shouldn't be ${createdApp?.scmType} before "Configure Deployment Source to None".`
		);
		await runWithTestActionContext(
			"ConfigureDeploymentSource",
			async (context) => {
				await context.ui.runWithInputs(
					[resourceName, constants.ScmType.None],
					async () => {
						await editScmType(context);
					}
				);
		...
		assert.strictEqual(
			createdApp?.scmType,
			constants.ScmType.None,
			`Web App scmType's property value should be ${constants.ScmType.None} rather than ${createdApp?.scmType}.`
		);
	});
	...
	test(`Add and delete settings for ${WebsiteOS0} Web App`, async () => {
		...
		await runWithTestActionContext("appSettings.Add", async (context) => {
			await context.ui.runWithInputs(
				[resourceName, appSettingKey, appSettingValue],
				async () => {
		...
		assert.strictEqual(
			await getAppSettingValue(resourceName, resourceName, appSettingKey),
			appSettingValue,
			`Fail to add setting "${appSettingKey}"`
		);
		await runWithTestActionContext(
			"appSettings.Delete",
			async (context) => {
				await context.ui.runWithInputs(
					[
						resourceName,
						`${appSettingKey}=Hidden value. Click to view.`,
	...
	test(`Delete Web App for ${WebsiteOS0} Web App`, async () => {
		...
		const deletedApp: Site | undefined = await tryGetWebApp(
			webSiteClient,
			resourceName,
			resourceName
		);
	...
	async function getAppSettingValue(
		resourceGroupName: string,
		webAppName: string,
		key: string
	): Promise<string | undefined> {
		let value: string | undefined;
		const listAppSettings: StringDictionary =
			await webSiteClient.webApps.listApplicationSettings(
				resourceGroupName,
				webAppName
			);
	...
	function getInput(inputOS: WebsiteOS): (string | RegExp)[] {
		...
		const webAppOS: string =
			inputOS.charAt(0).toUpperCase() + inputOS.slice(1);
